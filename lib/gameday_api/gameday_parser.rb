  ...
  class GamedayParser
  	...
  	def self.parse_innings_ab_data(doc, game_id)
  		...
  		doc.root.each_element do |inning_half|
            ...
            atbat = set_atbat_from_at_bat_action(atbat_action)
            atbat.inning = inning
            atbat.half = half
            current_atbat = atbat.number
						atbat.batter_id = batter.id if batter
						atbat.pitcher_id = pitcher.id if pitcher
						...
						atbat_action.each_element do |pitch_runner|
							if pitch_runner.name == 'pitch'
								pitch = set_pitch_from_pitch_runner(pitch_runner)
                ...
                runner = set_runner_from_pitch_runner(pitch_runner)
						...
						action = set_action_from_atbat_action(atbat_action)
...
# TODO: Check to see if Ruby can handle this end in here. I'm thinking it' going to be reserved,
# but I can't imagine that it was working before.
    def self.set_runner_from_pitch_runner(pitch_runner)
      runner = Runner.new
    ...
    def self.set_pitch_from_pitch_runner(pitch_runner)
      pitch = Pitch.new
    ...
    def self.set_atbat_from_atbat_action(atbat_action)
      atbat = AtBat.new
    ...
    def self.set_action_from_atbat_action(atbat_action)
      action = AtBat.new
